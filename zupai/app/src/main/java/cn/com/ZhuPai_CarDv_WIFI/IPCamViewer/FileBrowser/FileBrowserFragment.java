...
import cn.com.ZhuPai_CarDv_WIFI.IPCamViewer.RTPullListView;
import cn.com.ZhuPai_CarDv_WIFI.IPCamViewer.StartRecord;
...
public class FileBrowserFragment extends Fragment {
    ...
    private static boolean isOpenFile=false;//记录打开操作
    private static boolean isdownloadsleep=false;// 记录锁屏下载完成后是否有操作
    ...
    private boolean returnRecording=false;//待机自动录像
...
//					mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_GETFILELIST),G_TRYTIME);
    ...
    private class BackViedoDownFileListTask extends AsyncTask<FileBrowser,Integer,FileBrowser>{
        ...
        protected void onPostExecute(FileBrowser result) {
            ...
            if(activity!=null){
                List<FileNode>fileNodeList=result.getFileList();
                Log.d(TAG,"后路视频请求返文件个数："+fileNodeList.size());
                for (int i = 0; i < fileNodeList.size(); i++) {
    ...
    private class SOSFrontViedoDownFileListTask extends AsyncTask<FileBrowser,Integer,FileBrowser>{
        ...
        protected void onPostExecute(FileBrowser result) {
            ...
            if(activity!=null){
                ...
                mFileListSOSFrontVIDEOAdapter.notifyDataSetChanged();
                if (!result.isCompleted()&&fileNodeList.size()>0){
                    Log.d(TAG,"SOS视频返回文件数："+sFilelistSOSFrontVIDEO.size());
                    mFileListSOSFrontVIDEOAdapter.notifyDataSetChanged();
                    mHandler.sendEmptyMessage(MSG_DISMISSDIALOG);
                    moreProgressBar.setVisibility(View.GONE);
                }
...
//					mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_GETFILELIST),G_TRYTIME);
    ...
    private static FileListJpgAdapter mFileListJPGAdapter ;
    private static FileListVideoAdapter mFileListVIDEOAdapter ;
    private static FileListVideoAdapter mFileListBACKVIDEOAdapter;
    private static FileListVideoAdapter mFileListSOSFrontVIDEOAdapter;
...
//					}
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mFileListJPGAdapter = new FileListJpgAdapter(inflater, sFileListJPG) ;
        mFileListVIDEOAdapter = new FileListVideoAdapter(inflater, sFileListVIDEO) ;
        mFileListBACKVIDEOAdapter = new FileListVideoAdapter(inflater, stackFileListBACKVIDEO) ;
        mFileListSOSFrontVIDEOAdapter=new FileListVideoAdapter(inflater,sFilelistSOSFrontVIDEO);
        ...
        myOnTouchListener=new MainActivity.MyOnTouchListener() {
            ...
            public boolean onTouch(MotionEvent ev) {
                ...
                if (isINFilebrowser&&!isDownloadFile){
                    isTounch = true;
                    sleepHandler.removeMessages(1);
...
//					Toast.makeText(getActivity(), R.string.pleaseSelectFile,
//							Toast.LENGTH_SHORT).show();
                    toastUtil(getResources().getString(R.string.pleaseSelectFile));
                }
//				mSaveButton.setEnabled(false) ;
...
//		sSelectedFiles.clear() ;
    ...
    static	private Handler sleepHandler = new Handler()
    {
        public void handleMessage(Message msg){
            switch(msg.what)
            {
                case 1:
                    if (!isDownloadFile&&!isOpenFile){
                        isDownload=false;
                        Log.i(TAG, "触摸无操作，返回主界面"+isDownloadFile+isOpenFile);
                        break;
                    }
                    Log.i(TAG, "触摸--!isDownloadFile="+!isDownloadFile+isOpenFile);
                case 2:
    ...
    public static void isdownloadsleep(){
        TimerTask task=new TimerTask() {
            @Override
            public void run() {
                if (!isDownloadFile&&isINFilebrowser && !isTounch) {
                    Log.i(TAG, "下载完成后有操作，不返回主界面"+isDownloadFile+isINFilebrowser+isTounch);
                    isDownload=false;
                }else {
                    Log.i(TAG, "下载完成后无操作，返回主界面"+isDownloadFile+isINFilebrowser+isTounch);
                }
            }
        };timer.schedule(task, 1000 * MainActivity.DOUWNTIME);
    }
    private class TimeThread extends Thread{
        public synchronized void run() {
            while(isDownload)
            {
                try{
                    Thread.sleep(1000);
//					Log.i(TAG, "下载完成后无操作，返回主界面---");
                } catch (Exception e){
                    e.printStackTrace();
                }
            }
            returnRecording=true;
            returnFragment =true;
            if (returnBlankScreen){
                Log.i(TAG, "下载完成开启录像");
                Looper.prepare();
                new StartRecord().startRecord();
//				new BackGetRecordStatus().execute();
            }else {
                Log.i(TAG, "TimeThread返回主界面");
                getFragmentManager().popBackStack();
            }
        }
    };
